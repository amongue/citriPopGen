...
pc14[which(!is.na(pc14$Ericerus_ortholog)),12]<-"Ericerus"
...
#weakly deleterious = PN<0.2 - (PN>0.2 *PS<0.2)/PS>0.2
#from our data we have PN>0.2 and PS>0.2 already, to get the number of low freq variants, we need to do
pnl<-pc15$Pn - pc15$Pn.2
psl<-pc15$Ps - pc15$Ps.2
weadel<-round(pnl - ((pc15$Pn.2*psl)/pc15$Ps.2))
#why does that sometimes give us negatives?
#we'll consider element 35
#Pn    Ps    Pn.2   Ps.2 so Pn<0.2  Ps<0.2
#2     2     2      1       0       1
#by our math
# weadel = 0 - (2 *1)/1
#this suggests that some Pn counts will actually *increase* with this correction??

#alternatively
#the fww extended says p-neut= Ps<0.2/Ps; Pn-neut=Pn>0.2 + p-neut*Pn<0.2
pnneut<-round(pc15$Pn.2 + (psl/pc15$Ps)*pnl)
pc15<-as.data.frame(cbind(pc15,pnneut))
...
pN.pS.neut<-(pc15$pnneut/pc15$Non.sites)/(pc15$Ps/pc15$Syn.sites)
...
extended.alpha<-1-(pN.pS.neut/dN.dS)
...
dt<-dunnTest((pc16$Pn/pc16$Non.sites)~pc16$consistency)
dtr<-dt$res
...
#let's check for bias across the genome
sexdist<-table(pc17$consistency,pc17$scaffold)

table(pc17$consistency,pc17$scaffold)/c(1972,1972,1972,1455,1455,1455,1453,1453,1453,1381,1381,1381,1061,1061,1061)
chisq.test(sexdist)
...
#ugly way to plot points, but we're in a hurry
pc17fb<-pc17[which(pc17$consistency=="female_biased"),]
fbpn<-(pc17fb$Pn/pc17fb$Non.sites)
fbps<-(pc17fb$Ps/pc17fb$Syn.sites)
fbpnps<-fbpn/fbps
pc17ub<-pc17[which(pc17$consistency=="unbiased"),]
ubpn<-(pc17ub$Pn/pc17ub$Non.sites)
ubps<-(pc17ub$Ps/pc17ub$Syn.sites)
ubpnps<-ubpn/ubps
pc17mb<-pc17[which(pc17$consistency=="male_biased"),]
mbpn<-(pc17mb$Pn/pc17mb$Non.sites)
mbps<-(pc17mb$Ps/pc17mb$Syn.sites)
mbpnps<-mbpn/mbps
...
boxplot((pc17$Pn/pc17$Non.sites)~pc17$consistency,outline=F,notch=T,ylab="",xlab="Sex-bias in expression",las=1,
        ...
        cex.main=1.6,main="",names=c("F","UB","M"),ylim=c(0,0.013))
stripchart(fbpn,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=1,vertical=T)
stripchart(ubpn,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=2,vertical=T)
stripchart(mbpn,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=3,vertical=T)
...
boxplot((pc17$Ps/pc17$Syn.sites)~pc17$consistency,outline=F,notch=T,ylab="",xlab="Sex-bias in expression",las=1,
        col=c("firebrick3","grey33","dodgerblue3"),cex.lab=1.2,cex.axis=1.2,
        cex.main=1.6,main="",names=c("F","UB","M"),ylim=c(0,0.083))
mtext("pS", side = 3, line = 1, cex = 2)
stripchart(fbps,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=1,vertical=T)
stripchart(ubps,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=2,vertical=T)
stripchart(mbps,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=3,vertical=T)
...
boxplot((pc17$pN.pS)~pc17$consistency,outline=F,notch=T,ylab="",xlab="Sex-bias in expression",las=1,
        col=c("firebrick3","grey33","dodgerblue3"),cex.lab=1.2,cex.axis=1.2,
        cex.main=1.6,main="",names=c("F","UB","M"),ylim=c(0,1.36))
stripchart(fbpnps,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=1,vertical=T)
stripchart(ubpnps,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=2,vertical=T)
stripchart(mbpnps,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=3,vertical=T)
...
#again ugly code for individual gene plotting
fbdn<-(pc17fb$Dn/pc17fb$Non.sites)
fbds<-(pc17fb$Ds/pc17fb$Syn.sites)
fbdnds<-fbdn/fbds

ubdn<-(pc17ub$Dn/pc17ub$Non.sites)
ubds<-(pc17ub$Ds/pc17ub$Syn.sites)
ubdnds<-ubdn/ubds

mbdn<-(pc17mb$Dn/pc17mb$Non.sites)
mbds<-(pc17mb$Ds/pc17mb$Syn.sites)
mbdnds<-mbdn/mbds
#2.4 the Figure, Figure 3
layout( matrix(c(1,2,3,3), nrow=2, byrow=TRUE) )
#par(mai=c(bottom,left,top.right))
par(mai=c(0.65,0.9,0.7,0.06))
...
stripchart(fbdn,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=1,vertical=T)
stripchart(ubdn,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=2,vertical=T)
stripchart(mbdn,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=3,vertical=T)
...
stripchart(fbds,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=1,vertical=T)
stripchart(ubds,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=2,vertical=T)
stripchart(mbds,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=3,vertical=T)
...
stripchart(fbdnds,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=1,vertical=T)
stripchart(ubdnds,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=2,vertical=T)
stripchart(mbdnds,method="jitter",pch=19,col=rgb(red=38,green=38,blue=38,alpha=20,maxColorValue = 255),add=T,at=3,vertical=T)
...
#3.3 the plot, Figure 4
par(mfrow=c(1,2))
boxplot(pc17$alpha.simple~pc17$consistency,outline=F,notch=T,ylab="",xlab="",las=1,
        col=c("firebrick3","grey33","dodgerblue3"),cex.lab=1.2,cex.axis=1.2,
        cex.main=1.6,main="",names=c("F","UB","M"),ylim=c(-3.8,1))
abline(h=0)
mtext(expression(alpha), side = 2, line = 2.6, cex = 3)
...
boxplot(pc17$alpha.impmkt2~pc17$consistency,outline=F,notch=T,ylab="",xlab="",las=1,
        col=c("firebrick3","grey33","dodgerblue3"),cex.lab=1.2,cex.axis=1.2,
        cex.main=1.6,main="",names=c("F","UB","M"),ylim=c(-0.7,1))
abline(h=0)
mtext("Sex bias in expression",side=1, line=2,adj=0.5,cex=1.5)
...
#GO term parsing
anns<-fread("C:/Users/andre/Documents/Documents/Documents/CitriPopgen/interproscan.csv",stringsAsFactors = F)

pcgo<-merge(pc17,anns,by=c("gene","transcript"))
pcgou<-pcgo[which(pcgo$consistency=="unbiased"),]
pcgom<-pcgo[which(pcgo$consistency=="male_biased"),]
pcgof<-pcgo[which(pcgo$consistency=="female_biased"),]

gom<-table(pcgom$`GO Terms`)
gou<-table(pcgou$`GO Terms`)
gof<-table(pcgof$`GO Terms`)

goms <- gom[order(gom, decreasing=TRUE)]
gous <- gou[order(gou, decreasing=TRUE)] 
gofs <- gof[order(gof, decreasing=TRUE)]

gomt<-as.data.table(gom,stringsAsFactors=F)
colnames(gomt)<-c("Go","M")
goft<-as.data.table(gof,stringsAsFactors=F)
colnames(goft)<-c("Go","F")
gout<-as.data.table(gou,stringsAsFactors=F)
colnames(gout)<-c("Go","U")
dum<-merge(goft,gout,by="Go",all.x=T,all.y=T)
fullgo<-merge(dum,gomt,by="Go",all.x=T,all.y=T)
#now we need a way to quickly look for enrichment...we could just look at the GO terms most skewed
#that will require some normalization though
#first we need to get rid of NAs for subsequent analysis
fullgo[is.na(fullgo)] <- 0.0001
#next we need to normalize by number of genes per sex-bias category
normf<-fullgo$F/colSums(fullgo[,2])
normu<-fullgo$U/colSums(fullgo[,3])
normm<-fullgo$M/colSums(fullgo[,4])
#that gives a proportion within bias class
#now we calculate enrichment as a how common it is within a class relative to between classes
fullnorms<-as.data.frame(cbind(normf,normu,normm),stringsAsFactors = F)
finalnorm <-rowSums(fullnorms)
#it's ugly but it works, divide each sample-size normalized count by the between class counts
fgnor<-as.data.frame(cbind(fullgo,normf/finalnorm,normu/finalnorm,normm/finalnorm),stringsAsFactors=F)
colnames(fgnor)<-c("GO","F","U","M","FN","UN","MN")
#this works
head(fgnor[order(fgnor$FN, decreasing=TRUE),],10)
head(fgnor[order(fgnor$UN, decreasing=TRUE),],10)
head(fgnor[order(fgnor$MN, decreasing=TRUE),],10)



####looking at biased codon usage
thirds<-fread("C:/Users/andre/Documents/Documents/Documents/CitriPopgen/degenerate_codons.csv",header=T,stringsAsFactors=F)

third_sum<-reshape2::dcast(thirds, transcript ~ codon.base)
#we'll split the gene and transcript in excel
#write.csv(third_sum,"C:/Users/andre/Documents/Documents/Documents/CitriPopgen/gene4x3rdpos.csv",quote=F,row.names=F)
thirdfinal<-as.data.frame(fread("C:/Users/andre/Documents/Documents/Documents/CitriPopgen/gene4x3rdpos_split.csv"),stringsAsFactors=F)
codtest<-merge(pc17,thirdfinal,by=c("gene","transcript"))
codm<-codtest[which(codtest$consistency=="male_biased"),52:55]
codms<-colSums(codm)
codu<-codtest[which(codtest$consistency=="unbiased"),52:55]
codus<-colSums(codu)
codf<-codtest[which(codtest$consistency=="female_biased"),52:55]
codfs<-colSums(codf)

overalls<-colSums(thirdfinal[,3:6])
overalls/sum(overalls)
#this is our baseline
#A         C         G         T 
#0.2816757 0.1945023 0.2017334 0.3220886

codms/sum(codms)
#male-biased
#A         C         G         T 
#0.2958763 0.1958959 0.2012131 0.3070147 
# ^           -       -        v
...
#unbiased
#A         C         G         T 
#0.2584960 0.1994225 0.2095620 0.3325194 
#   vv      -        -          ^
codfs/sum(codfs)
#female-biased
#A         C         G         T 
#0.3157067 0.1931886 0.1817675 0.3093372 
#  ^        -         v           V
endcod<-as.data.frame(rbind(codfs,codus,codms))

chisq.test(endcod)
#X-squared = 4105.7, df = 6, p-value < 2.2e-16

endcod<-as.data.frame(rbind(codfs,codms))
chisq.test(endcod)
#X-squared = 342.44, df = 3, p-value < 2.2e-16
